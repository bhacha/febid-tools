...
def prepare_stream(self, SEM_settings, min_dwell=10, max_dwell=8e5):
    ...
    print(f"Structure Size nm : {self._strucure.structure_size_nm}")
...
def output_stream(self, filename, sample_factor=1):
    ...
    with open(filename+'.str', 'w') as f:
        ...
        for k in range(0,int(numpoints), sample_factor):
            ...
            if k < int(numpoints)-sample_factor:
                f.write(linestring + '\n')
            else:
